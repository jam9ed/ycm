...
// ---- the markup the page ships -------------------------------------------
{
  ...
  ok('it carries four clips', clips.length === 4, clips.length + '');
...
// ---- what it builds -------------------------------------------------------
{
  ...
  ok('the next clip is queued behind it', /acc9102c95/.test(idle.getAttribute('src') || ''), idle.getAttribute('src'));
  ...
  ok('and queues the one after', /464cfe6e71/.test(on[0].getAttribute('src')), on[0].getAttribute('src'));
...
// ---- weight ---------------------------------------------------------------
{
  ...
  ok('the whole loop stays under 10MB', bytes < 10 * 1024 * 1024,
